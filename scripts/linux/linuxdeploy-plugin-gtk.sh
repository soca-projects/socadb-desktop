...
# (originally adapted by https://github.com/khoj-ai/pipali) with a patch
# to handle statically-linked / Bun-compiled binaries anywhere under
# AppDir/usr/.
#
# Why this exists for SocaDB:
# Tauri's pinned linuxdeploy walks every executable file in the AppDir and
# runs `ldd` on it to bundle dependencies. When ldd returns exit code 1 on
# a static / Bun-compiled / single-file binary it crashes with
# `std::runtime_error: Failed to run ldd` and aborts the AppImage build
# (SIGABRT, exit 134). This blocks our Linux release because we ship the
# Anthropic SDK's bundled Claude binary (`@anthropic-ai/claude-agent-sdk-linux-x64/claude`)
# and our own Bun-compiled MCP server under usr/lib/SocaDB/runtime/.
#
# This script overrides Tauri's cached linuxdeploy-plugin-gtk to move static
# binaries out of the AppDir before linuxdeploy scans them, run linuxdeploy
# on the remaining (dynamically linked) binaries, then restore the static
# ones in their original relative location.
#
# References:
#   https://github.com/linuxdeploy/linuxdeploy/issues/147
#   https://github.com/tauri-apps/tauri/issues/8604
#   https://github.com/khoj-ai/pipali/blob/main/scripts/linux/linuxdeploy-plugin-gtk.sh
#
# When this can be removed: when linuxdeploy upstream stops crashing on
# unreadable-by-ldd binaries, or when we drop the bundled Claude/Codex/Bun
# binaries from the AppImage payload (e.g. by moving them out of usr/lib/
# or by switching to a user-installed CLI model).
...
# PATCH (SocaDB-specific): Move statically-linked / Bun-compiled binaries out
# of AppDir before linuxdeploy scans them. linuxdeploy runs `ldd` on every
# executable and crashes with std::runtime_error when ldd returns exit code 1
# (which happens on Bun --compile single-file binaries like ours). We move
# them out preserving their relative path so they can be restored to the
# correct subdirectory afterwards. Scope is "$APPDIR/usr" rather than just
# "$APPDIR/usr/bin" (the upstream Pipali patch) because our problem binaries
# live under usr/lib/SocaDB/runtime/, not usr/bin/.
STATIC_BACKUP="/tmp/linuxdeploy-static-backup-$$"
mkdir -p "$STATIC_BACKUP"
while IFS= read -r -d '' file; do
    if ! ldd "$file" > /dev/null 2>&1; then
        relative="${file#$APPDIR/}"
        echo "Temporarily moving static binary out of AppDir: $relative"
        mkdir -p "$STATIC_BACKUP/$(dirname "$relative")"
        mv "$file" "$STATIC_BACKUP/$relative"
    fi
done < <(find "$APPDIR/usr" -executable -type f -print0)

env LINUXDEPLOY_PLUGIN_MODE=1 "$LINUXDEPLOY" --appdir="$APPDIR" "${LIBRARIES[@]}"

# PATCH (SocaDB-specific): Restore statically-linked binaries to their
# original relative paths inside AppDir.
if [ -d "$STATIC_BACKUP" ] && [ -n "$(ls -A "$STATIC_BACKUP")" ]; then
    echo "Restoring statically-linked binaries to AppDir"
    (cd "$STATIC_BACKUP" && cp -a . "$APPDIR/")
    rm -rf "$STATIC_BACKUP"
fi
